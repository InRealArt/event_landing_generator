import { notFound } from 'next/navigation'
import Header from '@/components/Header'
import HeroSection from '@/components/HeroSection'
import AboutSection from '@/components/AboutSection'
import PortraitSection from '@/components/PortraitSection'
import InRealArtStats from '@/components/InRealArtStats'
import ArtworkGallery from '@/components/ArtworkGallery'
import CatalogSection from '@/components/CatalogSection'
import GoogleCaptchaWrapper from '@/components/captcha/googleCaptchaWrapper'
import { getArtistData, getAllArtistSlugs } from '@/lib/artistDataManager'
import type { Metadata } from 'next'

interface PageProps {
  params: Promise<{ slug: string }>
}

// Génération des métadonnées dynamiques
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  const { slug } = await params
  const artistData = await getArtistData(slug)
  
  if (!artistData) {
    return {
      title: 'Artiste non trouvé',
      description: 'Cette page d\'artiste n\'existe pas.'
    }
  }

  return {
    title: artistData.seo.title,
    description: artistData.seo.description,
    keywords: artistData.seo.keywords.join(', '),
    authors: [{ name: artistData.fullName }],
    openGraph: {
      title: artistData.seo.title,
      description: artistData.seo.description,
      type: 'website',
      images: [
        {
          url: artistData.profileImage,
          width: 800,
          height: 600,
          alt: artistData.fullName,
        },
      ],
    },
    twitter: {
      card: 'summary_large_image',
      title: artistData.seo.title,
      description: artistData.seo.description,
      images: [artistData.profileImage],
    },
  }
}

// Génération des paramètres statiques
export async function generateStaticParams() {
  const slugs = await getAllArtistSlugs()
  return slugs.map((slug) => ({
    slug: slug,
  }))
}

// Configuration pour les paramètres dynamiques
export const dynamicParams = true

export default async function ArtistPage({ params }: PageProps) {
  const { slug } = await params
  const artistData = await getArtistData(slug)

  if (!artistData) {
    notFound()
  }

  return (
    <main className="min-h-screen bg-white">
      <Header artistData={artistData} />

      <HeroSection artistData={artistData} />

      <div id="gallery">
        <ArtworkGallery artistData={artistData} />
      </div>

      <div id="about">
        <AboutSection artistData={artistData} />
      </div>

      <PortraitSection artistData={artistData} />

      <div id="stats">
        <InRealArtStats artistData={artistData} />
      </div>

      <div id="contact">
        <GoogleCaptchaWrapper>
          <CatalogSection artistData={artistData} slug={slug} />
        </GoogleCaptchaWrapper>
      </div>

      {/* Footer */}
      <footer className="bg-gray-900 py-12 px-4">
        <div className="max-w-[80%] md:max-w-7xl mx-auto">
          {/* Main Footer Content */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-8">
            {/* Artist Info */}
            <div className="text-center md:text-left">
              <h3 className="text-xl font-bold mb-4 text-white font-bricolage">
                {artistData.fullName}
              </h3>
              <p className="text-gray-300 text-sm font-montserrat mb-4">
                {artistData.content.footer.artistDescription}
              </p>
            </div>

            {/* InRealArt Info */}
            <div className="text-center">
              <h3 className="text-xl font-bold mb-4 text-white font-bricolage">
                InRealArt
              </h3>
              <p className="text-gray-300 text-sm font-montserrat mb-4">
                {artistData.content.footer.inrealArtDescription}
              </p>
              <div className="space-y-2 text-gray-400 text-sm font-montserrat">
                <p>15+ Artistes sélectionnés</p>
                <p>Le catalogue c&apos;est 180+ oeuvres soigneusements choisies...</p>
                <p>100% De nos artistes satisfaits de leurs accompagnements</p>
              </div>
            </div>

            {/* Contact Info */}
            <div className="text-center md:text-right">
              <h3 className="text-xl font-bold mb-4 text-white font-bricolage">
                Contact
              </h3>
              <div className="flex flex-col gap-3 space-y-3 text-gray-300 text-sm font-montserrat">
                <div className="flex justify-center md:justify-end space-x-4">
                  {/* LinkedIn */}
                  {artistData.socialMedia?.linkedin && (
                    <a
                      href={`https://linkedin.com/${artistData.socialMedia.linkedin}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-gray-400 hover:text-white transition-colors"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
                      </svg>
                    </a>
                  )}

                  {/* Facebook */}
                  {artistData.socialMedia?.facebook && (
                    <a
                      href={`https://facebook.com/${artistData.socialMedia.facebook}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-gray-400 hover:text-white transition-colors"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                      </svg>
                    </a>
                  )}

                  {/* Pinterest */}
                  {artistData.socialMedia?.pinterest && (
                    <a
                      href={`https://pinterest.com/${artistData.socialMedia.pinterest}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-gray-400 hover:text-white transition-colors"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.174-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.402.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.357-.629-2.746-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24.009 12.017 24.009c6.624 0 11.99-5.367 11.99-11.988C24.007 5.367 18.641.001 12.017.001z" />
                      </svg>
                    </a>
                  )}

                  {/* Instagram */}
                  {artistData.socialMedia?.instagram && (
                    <a
                      href={`https://instagram.com/${artistData.socialMedia.instagram.replace('@', '')}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-gray-400 hover:text-white transition-colors"
                    >
                      <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
                      </svg>
                    </a>
                  )}
                </div>
                <div>
                  <a
                    href="https://calendly.com/teaminrealart/demande-de-rdv"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center px-6 py-3 bg-purple-600 text-white font-medium rounded-lg hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 transition-colors font-montserrat"
                  >
                    <span>Prendre RDV</span>
                    <svg className="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                  </a>
                </div>
              </div>
            </div>
          </div>

          {/* Bottom Border */}
          <div className="border-t border-gray-700 pt-8">
            <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
              <p className="text-gray-400 text-sm font-montserrat">
                {artistData.content.footer.copyright}
              </p>
              <div className="flex space-x-6 text-gray-400 text-sm font-montserrat">
                <a href="https://policies.google.com/privacy" className="hover:text-white transition-colors">
                  {artistData.content.footer.legalLinks.privacy}
                </a>
                <a href="https://www.inrealart.com/terms" className="hover:text-white transition-colors">
                  {artistData.content.footer.legalLinks.terms}
                </a>
              </div>
            </div>
          </div>
        </div>
      </footer>
    </main>
  )
}
